...
Каждая карточка вида спорта отображает иконку. Логика выбора (в порядке приоритета):

1. **Картинка с сервера** — если поле `image_url` в ответе `/v_2/sports` не пустое, отображается `<img src={image_url}>`. Файлы хранятся на сервере и возвращаются напрямую бэкендом; URL прописывается в таблице `ref_sports.image_url`.
2. **Эмодзи-фоллбек** — если `image_url` пустой или картинка не загрузилась (событие `onError`), отображается эмодзи из `src/utils/sportEmojis.js`.

Чтобы добавить иконку для вида спорта: загрузи файл на сервер и обнови поле `image_url` в таблице `ref_sports` через административный интерфейс или напрямую в БД.
...
- ~~**Нет авторизации**~~ — решено: HTTP Basic Auth на `/admin` через Nginx (`.htpasswd` монтируется в контейнер, не хранится в git).
